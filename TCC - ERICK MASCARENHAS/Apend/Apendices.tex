...
Cada participante realiza uma condição por sessão, na ordem definida pelo quadrado de Williams. A exposição dura 20 minutos nas quatro condições. As instruções abaixo são lidas imediatamente antes da exposição.
...
A ideia é simplesmente jogar de forma tranquila por vinte minutos. Se sentir qualquer desconforto, enjoo ou tontura, avise imediatamente e paramos na hora. Podemos começar?
\end{quote}
...
O objetivo é acompanhar o ritmo e acertar o máximo que conseguir. Se a partida terminar antes dos vinte minutos, ela recomeça automaticamente até completar o tempo.
...
\begin{quote}\itshape
Nesta sessão não há jogo nem óculos. Você vai apenas ouvir música por vinte minutos, sentado, com os fones. É a mesma trilha que aparece em uma das outras sessões.
...
\noindent\textbf{[A]} Iniciar a reprodução pelo mesmo aplicativo empregado em C1, em modo C3, executado no computador do pesquisador com a tela escurecida. A playlist, sua ordem e seu volume são idênticos aos de C1, e os marcadores de início e fim são emitidos automaticamente.
...
\noindent\textbf{[A]} Procedimento de higienização e ajuste do \gls{hmd} idêntico ao de C1. Encerrar o aplicativo do \textit{Cozy Pong} e abrir o aplicativo de meditação previamente definido, já posicionado na sessão que será utilizada. Conferir que o dispositivo está em modo avião ou sem rede, para impedir atualização de conteúdo durante a coleta, e que o volume do sistema está no valor padronizado.

\noindent\textbf{[A]} Iniciar, no computador do pesquisador, o cronômetro de exposição do aplicativo desenvolvido neste trabalho, em modo C4. Ele não reproduz áudio nem imagem: registra os marcadores temporais e sinaliza o fim dos 20 minutos, de modo que a marcação desta condição tenha a mesma precisão das demais.

\begin{quote}\itshape
Nesta sessão você vai usar os óculos, mas não há jogo. Você verá um ambiente virtual tranquilo e ouvirá uma narração que guia um exercício de respiração, por vinte minutos.
...
\noindent\textbf{[A]} Iniciar a sessão no aplicativo de meditação e o cronômetro no mesmo instante. Marcar o início da exposição. Permanecer em silêncio durante toda a sessão.

\noindent\textbf{[D]} Se a sessão do aplicativo terminar antes dos 20 minutos, iniciar imediatamente a sessão seguinte previamente definida, sem interagir com o participante, e registrar o instante da transição. Se ultrapassar os 20 minutos, encerrar no cronômetro e registrar o ponto de corte.

\subsection*{Padronização do aplicativo de meditação}

Diferentemente de C1, C2 e C3, esta condição não é conduzida pelo aplicativo desenvolvido neste trabalho. A escolha e sua justificativa constam da Seção~\ref{subsec:meditacao-vr}, e a limitação decorrente, da Seção~\ref{sec:limitacoes}. Para que a condição seja idêntica entre participantes, ficam fixados antes do início da coleta e registrados no protocolo:

\begin{enumerate}[label=\alph*)]
    \item o aplicativo e o número exato de sua versão;
    \item a sessão ou faixa utilizada dentro dele, sempre a mesma, e a sequência de sessões a empregar caso a primeira termine antes do tempo;
    \item o volume do sistema no dispositivo, aferido e anotado;
    \item as configurações internas do aplicativo que afetem conteúdo ou apresentação, como voz, idioma, música de fundo e ambiente visual;
    \item a ausência de conexão de rede durante todas as sessões, de modo a impedir atualização de conteúdo ao longo da coleta.
\end{enumerate}

\noindent\textbf{[A]} Ao final da coleta, conferir que a versão do aplicativo permanece a mesma registrada no início. Qualquer atualização involuntária deve ser documentada, com a indicação de quais participantes foram expostos a cada versão.

\todo{registrar aqui o aplicativo, a versao, a sessao utilizada e o volume padronizado, apos a definicao e a verificacao da licenca de uso para pesquisa.}

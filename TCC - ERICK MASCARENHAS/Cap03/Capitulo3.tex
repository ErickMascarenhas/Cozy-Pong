...
A avaliação de cada rebatida combina dois fatores, a força e o tempo. A força é estimada pela velocidade relativa no contato entre bola e raquete: velocidades muito altas são classificadas como excessivas e velocidades muito baixas como fracas. O tempo é o \emph{erro de sincronização} do evento, isto é, a menor distância entre o instante da rebatida e alguma batida da música, exatamente a grandeza $e_i$ definida na Equação~\ref{eq:sinc} e reportada nos resultados. Adota-se uma janela estreita para o acerto ideal e uma janela mais larga para o acerto tolerável. O resultado final de cada rebatida é uma de quatro categorias, definidas no tipo \texttt{HitType}: \textit{Perfect} (força e tempo ideais), \textit{Ok} (força fraca ou tempo tolerável), \textit{Bad} (força excessiva ou tempo fora da janela) e \textit{Miss} (bola não rebatida, que atinge o chão, ou bola proibida rebatida). A Tabela~\ref{tab:hittype} sintetiza os limiares adotados na configuração de referência.

Cabe explicitar por que a grandeza julgada pelo jogo e a grandeza reportada no trabalho são deliberadamente a mesma. Uma implementação alternativa, e mais simples, julgaria o tempo pela diferença entre o voo efetivo da bola e o voo previsto; a quantidade resultante é próxima do erro de sincronização, mas não idêntica a ele. Fazer o jogo pontuar uma coisa e o texto relatar outra criaria uma discrepância sem contrapartida, e por isso ambas as funções derivam do mesmo cálculo, sobre a lista completa de batidas do mapa rítmico.

\begin{table}[H]
    \centering
    \caption{Categorias de rebatida (\texttt{HitType}) e os limiares de força e erro de sincronização adotados na configuração relaxante. Os limiares de tempo da configuração animada constam da Tabela~\ref{tab:parametros}.}
    \label{tab:hittype}
    \small
    \begin{tabularx}{\textwidth}{p{2.2cm}Xp{3.2cm}}
        ...
        \midrule
        \textit{Perfect} & Erro de sincronização até $0{,}25$~s e força entre os limiares & $+200$ pontos, $+20$ de vida \\
        \textit{Ok}      & Erro até $0{,}50$~s ou força abaixo de $1{,}5$ & $+100$ pontos, $+5$ de vida \\
        \textit{Bad}     & Força acima de $3{,}5$ ou erro acima de $0{,}50$~s & $+50$ pontos, sem ganho de vida \\
...
A produção dos mapas é feita por um editor próprio, integrado ao projeto, que reproduz a música e permite marcar notas em tempo real por meio do teclado, atribuindo-lhes o tipo selecionado, além de salvar e recarregar mapas. Essa ferramenta viabiliza o mapeamento manual das faixas \textit{lo-fi} da versão pública do jogo.

Para as dez faixas empregadas no experimento, contudo, o mapeamento manual foi substituído por um procedimento assistido por análise de áudio, por três motivos. O mapeamento à mão introduz um desvio próprio de cada faixa, que varia com a atenção de quem mapeia; ele tende a cobrir apenas parte da canção, deixando trechos sem eventos; e adota subdivisões métricas distintas de faixa para faixa, o que impede parametrizar a densidade de forma uniforme.

O procedimento adotado estima, para cada faixa, a grade métrica do próprio áudio. Calcula-se a envoltória de intensidade de ataques (\textit{onset strength}) e busca-se, em torno do andamento nominal, o período que maximiza a magnitude do coeficiente de Fourier correspondente; a fase da grade é obtida do argumento desse mesmo coeficiente. O procedimento foi validado contra os mapas manuais já existentes: em $17$ das $24$ faixas examinadas, o desvio entre as notas manuais e a grade estimada apresentou dispersão inferior a $10$~ms, e a mediana desses desvios ficou em $+2{,}1$~ms, o que indica ausência de viés sistemático no método. Os desvios individuais dos mapas manuais, por sua vez, variaram de $-15$ a $+60$~ms entre faixas, confirmando a irregularidade que o procedimento elimina.

Determina-se ainda, batida a batida, se o áudio apresenta um ataque detectável, e delimita-se a região em que o pulso está efetivamente presente, de modo que não sejam lançadas bolas sobre introduções e desfechos sem percussão audível. Nas dez faixas selecionadas, essa região cobre em média $91\%$ da duração da canção, e a fração de notas apoiadas por um ataque detectável varia de $85\%$ a $99{,}7\%$. Os mapas resultantes são gravados no mesmo formato de texto do editor, com uma nota por batida.
...
Cada configuração emprega uma \textbf{playlist fixa de cinco faixas}, idêntica para todos os participantes na ordem, no conteúdo e no volume. Os dez mapas rítmicos correspondentes foram produzidos especificamente para o experimento, com \emph{uma nota por batida}, de modo que a densidade de bolas de uma faixa é simplesmente o seu andamento dividido pelo fator de subdivisão adotado, aqui fixado em quatro para todas as faixas. A Tabela~\ref{tab:parametros} consolida a parametrização resultante; as duas bandas de densidade não se sobrepõem.

A ordem das faixas dentro de cada configuração segue o princípio \textit{iso} da musicoterapia, segundo o qual a intervenção parte de um andamento próximo ao estado provável do ouvinte e o conduz gradualmente na direção pretendida. A configuração relaxante decresce de $92$ para $75$~BPM ao longo da exposição; a animada cresce de $110$ para $133$~BPM.

A playlist relaxante soma $1259$~s de áudio e cobre os $20$ minutos com folga, encerrando dentro da quinta faixa. A animada soma $1068$~s: a biblioteca disponível não possui cinco faixas rápidas longas o bastante para cobrir a exposição inteira, e faltam cerca de $132$~s. Nesse caso a última faixa é repetida, e não a primeira, para que o fecho da condição permaneça no ponto mais intenso do arco crescente em vez de retornar ao andamento inicial. A repetição é idêntica para todos os participantes e registrada na telemetria.

\begin{table}[H]
    \centering
    \caption{Parametrização das duas configurações do \textit{Cozy Pong} empregadas no experimento.}
    \label{tab:parametros}
    \small
    \begin{tabularx}{\textwidth}{p{5.2cm}YY}
        \toprule
        \textbf{Parâmetro} & \textbf{Relaxante (C1)} & \textbf{Animada (C2)} \\
        \midrule
        Faixas na playlist & 5 & 5 \\
        Andamento das faixas & 92 a 75 BPM & 110 a 133 BPM \\
        Ordem das faixas & decrescente & crescente \\
        Densidade de bolas & 23,0 a 18,8 por min & 27,5 a 33,3 por min \\
        Intervalo entre bolas & 2,61 a 3,20 s & 2,18 a 1,80 s \\
        Altura extra do arco & 0,25 m & 0,15 m \\
        Janela de vida da bola & 1,72 s & 1,40 s \\
        Limiar de sincronia \textit{Perfect} & 0,25 s & 0,15 s \\
        Limiar de sincronia \textit{Ok} & 0,50 s & 0,30 s \\
        Regime de erro & imortal & caixas de erro \\
        Placar, combo e nota & ocultos & visíveis \\
        \bottomrule
    \end{tabularx}\\[2pt]
    {\footnotesize Fonte: Elaborada pelo autor.}
\end{table}

Duas escolhas merecem justificativa. A altura do arco é menor na configuração animada porque é ela que determina a duração do voo: uma trajetória mais baixa é também mais rápida, o que atende à maior velocidade prevista para essa configuração e, ao mesmo tempo, evita que bolas consecutivas coexistam no ar quando o intervalo entre elas cai para menos de dois segundos. Já os limiares de julgamento são mais estreitos em C2 porque a exigência de precisão é parte do pacote de projeto que se pretende contrastar, e não um efeito colateral da densidade.

Esses valores estão declarados no código-fonte do aplicativo, e não em arquivos de configuração binários, de modo que permanecem versionados, comparáveis entre revisões e verificáveis por terceiros. Cada sessão registra, em seus metadados, a versão do aplicativo e os parâmetros efetivamente aplicados.

\todo{confirmar estes valores no estudo-piloto, verificando que a configuracao animada eleva a demanda sem tornar a tarefa frustrante, e ajustar a tabela se o piloto indicar mudanca.}

\subsection{Condição de Meditação Guiada em Realidade Virtual}\label{subsec:meditacao-vr}

A quarta condição experimental, a meditação guiada em \gls{rv} (Seção~\ref{sec:condicoes}), é conduzida por um \emph{aplicativo de meditação já estabelecido}, executado no mesmo \gls{hmd}, e não por um modo desenvolvido dentro do \textit{Cozy Pong}. A decisão é de validade de construto e de escopo. A condição existe para representar aquilo que hoje efetivamente se utiliza como prática de relaxamento digital; um exercício de meditação escrito e produzido pelo próprio autor, sem experiência clínica na técnica e sem evidência de eficácia, representaria mal esse comparador e transformaria o contraste C1~\textit{vs}~C4 em uma comparação entre duas produções amadoras. Empregar um aplicativo consolidado torna o comparador mais forte e o teste, por consequência, mais conservador.

A condição consiste em uma sessão guiada de foco na respiração, de duração idêntica à das demais condições, com o participante sentado e sem interação motora exigida. São padronizados entre participantes o aplicativo, a versão, a sessão escolhida dentro dele, o volume do dispositivo e a instrução verbal que a antecede, registrada no Apêndice~\ref{ap:condicoes}. O aplicativo é mantido \emph{offline} e sem atualização durante todo o período de coleta, de modo que o conteúdo apresentado ao último participante seja o mesmo apresentado ao primeiro.

Essa escolha tem um custo, registrado aqui e retomado na Seção~\ref{sec:limitacoes}: ao contrário de C1 e C2, a condição C4 não compartilha o binário, o \textit{pipeline} de renderização nem a cadeia de áudio do \textit{Cozy Pong}. O contraste C1~\textit{vs}~C4 passa, portanto, a comparar a atividade \emph{e} a implementação que a entrega, e não a atividade isoladamente. A marcação temporal é preservada por procedimento (Seção~\ref{sec:processamento}).

\todo{registrar o aplicativo escolhido, a versao exata, o titulo da sessao utilizada e sua duracao; confirmar que a licenca de uso permite emprego em pesquisa academica e anexar essa confirmacao ao processo do CEP.}
...
\begin{description}[style=nextline]
    ...
    O participante realiza, com o mesmo \gls{hmd}, uma sessão de meditação guiada com foco na respiração conduzida por aplicativo já estabelecido (Seção~\ref{subsec:meditacao-vr}), sentado e sem interação motora. Representa a prática de relaxamento digital hoje mais difundida e permite responder, com números, se o \textit{Cozy Pong} se equipara ou supera aquilo que já se utiliza. Compartilha com C1 e C2 o meio e o dispositivo, mas não o \textit{software}, ressalva discutida na Seção~\ref{sec:limitacoes}.
\end{description}

\begin{table}[H]
    \centering
    \caption{Atributos das quatro condições experimentais. C1 e C2 opõem dois pacotes de projeto (trilha e dificuldade variam em conjunto); C1 e C3 diferem na presença do jogo, mantida a mesma trilha; C1 e C4 diferem na atividade, mantidos o meio e o dispositivo.}
    \label{tab:condicoes}
    \small
    \begin{tabularx}{\textwidth}{p{2.5cm}YYYY}
        ...
        Trilha sonora & \textit{lo-fi} calma & animada, mais rápida & \textit{lo-fi} calma (idêntica a C1) & a do aplicativo de meditação \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{p{2.2cm}p{3.6cm}Y}
        ...
        \addlinespace
        C1 \textit{vs} C4 & atividade, entregue por outro \textit{software} & O jogo se equipara ou supera a meditação guiada em \gls{rv}? \\
        \bottomrule
    \end{tabularx}\\[2pt]
    {\footnotesize Fonte: Elaborada pelo autor.}
\end{table}
...
\begin{figure}[H]
    ...
    \label{fig:modos}
    \fbox{\begin{minipage}[c][5cm][c]{0.85\textwidth}\centering
        \textit{[Imagem a inserir: figura com quatro painéis: (a) participante com o HMD jogando Cozy Pong relaxante; (b) o mesmo jogo na configuração animada, com mais notas na tela; (c) participante sentado, de fones, apenas ouvindo a trilha; (d) participante com o HMD em sessão de meditação guiada.]}
    \end{minipage}}\\[2pt]
    {\footnotesize Fonte: Elaborada pelo autor.}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{p{0.8cm}p{2.7cm}Y}
        ...
        8 & 20 min & Exposição à condição designada, de duração idêntica nas quatro. Dados fisiológicos registrados continuamente e, em C1 e C2, também a telemetria do jogo. \\
...
Todos os sistemas são sincronizados por um mecanismo comum de marcação temporal, como o \gls{lsl}, um computador de aquisição compartilhado ou relógios sincronizados com marcadores explícitos de eventos. No mínimo, marcam-se o início e o fim da linha de base, do estressor, dos pontos de questionário, do início e do fim da exposição e da recuperação. Em C1, C2 e C3 o próprio aplicativo desenvolvido neste trabalho emite os marcadores, o que garante precisão idêntica nas três; em C4, conduzida por aplicativo de terceiros, o mesmo aplicativo é executado em paralelo, no computador do pesquisador, apenas como cronômetro e fonte de marcadores, de modo que a precisão da marcação seja igualmente idêntica. A precisão da sincronização é verificada em piloto antes do recrutamento.
...
A condição C4 é entregue por aplicativo de terceiros (Seção~\ref{subsec:meditacao-vr}), e não pelo mesmo binário que hospeda C1 e C2. Em troca de um comparador legítimo, conduzido por conteúdo profissionalmente produzido, o contraste C1~\textit{vs}~C4 passa a confundir a \emph{atividade} com a \emph{implementação} que a entrega: qualidade gráfica, latência, taxa de quadros, tratamento sonoro e produção de voz diferem entre as duas condições e não podem ser equalizados. A alternativa, produzir a meditação dentro do próprio projeto, eliminaria esse confundimento, mas ao custo de comparar o \textit{Cozy Pong} a um exercício de meditação sem evidência de eficácia, o que enfraqueceria justamente a pergunta que C4 existe para responder. Optou-se pelo comparador forte, e a ressalva permanece registrada. Padroniza-se o que é padronizável: aplicativo, versão, sessão, volume, postura e instrução verbal, mantidos idênticos entre participantes e congelados durante toda a coleta.

...
A Tabela~\ref{tab:parametros} consolida a parametrização numérica das duas configurações. A densidade de bolas de uma faixa resulta de dois fatores, o espaçamento das notas em seu mapa rítmico e o número de notas puladas entre dois lançamentos. Como o espaçamento não é uniforme entre os mapas produzidos, a maioria adotando uma nota por batida e alguns uma nota a cada duas batidas, o fator de subdivisão é definido faixa a faixa, e não globalmente, de modo a trazer todas as faixas de uma mesma configuração para a mesma banda de densidade. As duas bandas resultantes não se sobrepõem.

A ordem das faixas dentro de cada configuração segue o princípio \textit{iso} da musicoterapia, segundo o qual a intervenção parte de um andamento próximo ao estado provável do ouvinte e o conduz gradualmente na direção pretendida. A configuração relaxante decresce de $80$ para $71$~BPM ao longo da exposição; a animada cresce de $110$ para $133$~BPM.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{p{5.2cm}YY}
        ...
        \midrule
        Andamento das faixas & 71 a 80 BPM & 110 a 133 BPM \\
        Ordem das faixas & decrescente & crescente \\
        Densidade de bolas & 17,7 a 20,0 por min & 27,5 a 33,2 por min \\
        Intervalo entre bolas & 3,00 a 3,38 s & 1,81 a 2,18 s \\

...
public class WristMenuManager : MonoBehaviour
{
    ...
    private void OnToggleMenu(InputAction.CallbackContext context)
    {
        ...
        if (pauseOverlay)
        {
            if (IsMenuOpen && inGameObject != null && inGameObject.activeSelf) pauseOverlay.SetActive(true);
            else pauseOverlay.SetActive(false);
        }
        ServeManager activeManager = FindFirstObjectByType<ServeManager>();
        if (activeManager != null)
        {
            if (IsMenuOpen) activeManager.PauseGame();
            else activeManager.ResumeGame();
        }
        if (IsMenuOpen)
        {
